...
class Camera2Source {
    ...
    val isCamera2Native: Boolean
        get() {
            try {
                ...
                mCameraId = manager!!.cameraIdList[cameraFacing]
     ...
     */
        ...
        private val mCameraSource = Camera2Source()
    ...
    fun autoFocus(cb: AutoFocusCallback?, pEvent: MotionEvent, screenW: Int, screenH: Int) {
        if (cb != null) {
            mAutoFocusCallback = cb
        }
        if (sensorArraySize != null) {
            val y = pEvent.x.toInt() / screenW * sensorArraySize!!.height()
            val x = pEvent.y.toInt() / screenH * sensorArraySize!!.width()
    ...
    fun recordVideo(
        videoStartCallback: VideoStartCallback,
        videoStopCallback: VideoStopCallback,
        videoErrorCallback: VideoErrorCallback
    ) {
        try {
            ...
            if (mCameraDevice == null || !mTextureView!!.isAvailable || previewSize == null) {
    ...
    private fun getBestAspectPictureSize(supportedPictureSizes: Array<android.util.Size>): Size? {
        val targetRatio = Utils.getScreenRatio(mContext!!)
        var bestSize: Size? = null
        val diffs = TreeMap<Double, MutableList<android.util.Size>>()
        ...
        for (size in supportedPictureSizes) {
            ...
            if (diff < ratioTolerance) {
                if (diffs.keys.contains(diff)) {
                    ...
                    val newList = ArrayList<android.util.Size>()
                    newList.add(size)
                    diffs[diff] = newList
                }
            }
        ...
        for ((_, value) in diffs) {
            ...
            for (i in entries.indices) {
                val s = entries[i] as android.util.Size
                if (bestSize == null) {
    ...
    private fun setUpCameraOutputs(width: Int, height: Int) {
        try {
            ...
            mImageReaderStill = ImageReader.newInstance(
                largest!!.width,
                largest.height,
                ImageFormat.JPEG, /*maxImages*/
                2
            )
            mImageReaderStill!!.setOnImageAvailableListener(
            ...
            if (sensorOrientation != null) {
                ...
                when (mDisplayOrientation) {
                    ...
                    else -> Log.e(TAG, "Display rotation is invalid: $mDisplayOrientation")
                }
            }
            ...
            val displaySize =
                Point(
                    ...
                    Utils.getScreenHeight(mContext!!)
                )
            ...
            val outputSizes =
                Utils.sizeToSize(map.getOutputSizes(SurfaceTexture::class.java).map { Size(it.width, it.height) })
            val outputSizesMediaRecorder =
                Utils.sizeToSize(map.getOutputSizes(MediaRecorder::class.java).map { Size(it.width, it.height) })
            ...
            Log.d(TAG, "Camera Error: " + e.message)
        }
    ...
    private fun createCameraPreviewSession() {
        try {
            ...
            mImageReaderPreview = ImageReader.newInstance(
                previewSize!!.width,
                previewSize!!.height,
            ...
            mCameraDevice!!.createCaptureSession(
                listOf(surface, mImageReaderPreview!!.surface, mImageReaderStill!!.surface), object : CameraCaptureSession.StateCallback() {
    ...
    private fun createCameraRecordSession() {
        try {
            ...
            mCameraDevice!!.createCaptureSession(
                listOf(surface, mImageReaderPreview!!.surface, recorderSurface), object : CameraCaptureSession.StateCallback() {
                    override fun onConfigured(cameraCaptureSession: CameraCaptureSession) {
                        // The camera is already closed
    ...
    private inner class FrameProcessingRunnable internal constructor(private var mDetector: Detector<*>?) :
         ...
         */
        internal fun setNextFrame(data: ByteArray) {
            lock.withLock {
                ...
                lock.withLock {
                    ...
                    outputFrame = Frame.Builder()
                        .setImageData(
                            ...
                            ), previewSize!!.width / 4, previewSize!!.height / 4, ImageFormat.NV21
                        )
    ...
    private inner class PictureDoneCallback : ImageReader.OnImageAvailableListener {
    ...
    companion object {
        ...
        const val CAMERA_FLASH_OFF = CaptureRequest.CONTROL_AE_MODE_OFF
        const val CAMERA_FLASH_ON = CaptureRequest.CONTROL_AE_MODE_ON
        const val CAMERA_FLASH_AUTO = CaptureRequest.CONTROL_AE_MODE_ON_AUTO_FLASH
        const val CAMERA_FLASH_ALWAYS = CaptureRequest.CONTROL_AE_MODE_ON_ALWAYS_FLASH
        const val CAMERA_FLASH_REDEYE = CaptureRequest.CONTROL_AE_MODE_ON_AUTO_FLASH_REDEYE

        const val CAMERA_AF_AUTO = CaptureRequest.CONTROL_AF_MODE_AUTO
        const val CAMERA_AF_EDOF = CaptureRequest.CONTROL_AF_MODE_EDOF
        const val CAMERA_AF_MACRO = CaptureRequest.CONTROL_AF_MODE_MACRO
        const val CAMERA_AF_OFF = CaptureRequest.CONTROL_AF_MODE_OFF
        const val CAMERA_AF_CONTINUOUS_PICTURE = CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE
        const val CAMERA_AF_CONTINUOUS_VIDEO = CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_VIDEO

        private const val TAG = "Camera2Source"
        private const val ratioTolerance = 0.1
        private const val maxRatioTolerance = 0.18
